...
@section('content')
        ...
        <div class="option pb-5">
            <div class="image-option-games">
                <img src="{{$apartement->image}}" width="100%" ;
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            @if($apartement->discount)
                            @foreach($apartement->discount as $discount)
                    ...
                    <tbody>
                        <form method="post" onsubmit="myFunction()"
                            ...
                            @csrf
                            <tr>
                                <td data-label="اسم السيارة">{{$apartement->name_ar}}</td>
                                <td data-label="السعر"> {{$apartement->price}} </td>
                                @if($apartement->discount)
                                @foreach($apartement->discount as $discount)
                                <td> %{{$discount->rate}} </td>
                                @endforeach
